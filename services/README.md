...
### 1.1 `find_system_for_city(city, country_code)`
...
2. Otherwise, fetch data from `https://github.com/MobilityData/gbfs/blob/master/systems.csv` from GitHub (including +1500 cities)
...
### 1.2 `fetch_gbfs_feed_urls(system_id, auto_discovery_url)`
...
### 1.3 `fetch_station_information(station_information_url)`
...
### 1.4 `fetch_station_status(station_status_url)`
...
### 1.5 `get_stations_for_city(city_name)`
...
### 1.6 `get_vehicles_for_city(city_name)`
...
"vehicle_colors"    : { "SCOOTER": "#8e44ad", "EFIT": "#2980b9" } 
}
```
